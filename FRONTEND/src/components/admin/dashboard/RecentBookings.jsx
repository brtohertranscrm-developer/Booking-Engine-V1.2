          ...
          bookings.map((trx) => (
            <div 
              ...
              className={`bg-white rounded-3xl border transition-all duration-300 overflow-hidden ${
                expandedId === trx.id ? 'border-rose-200 shadow-md ring-4 ring-rose-50/50' : 'border-slate-100 hover:border-slate-300 shadow-sm'
              }`}
            >
              {/* === CARD HEADER (Selalu Tampil) === */}
              <div 
                onClick={() => toggleAccordion(trx.id)}
              ...
              >
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-lg shadow-inner flex-shrink-0 transition-colors ${
                    expandedId === trx.id ? 'bg-rose-500 text-white' : 'bg-slate-900 text-white group-hover:bg-slate-800'
                  }`}>
                ...
                <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4 sm:gap-6 border-t border-slate-100 sm:border-0 pt-4 sm:pt-0">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button className={`p-2 rounded-xl transition-colors ${
                      expandedId === trx.id ? 'bg-rose-100 text-rose-600' : 'bg-slate-50 text-slate-400 group-hover:text-slate-900 group-hover:bg-slate-100'
                    }`}>
              ...
              <div 
                className={`grid transition-all duration-300 ease-in-out ${
                  expandedId === trx.id ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                }`}
              >
                <div className="overflow-hidden">
                  <div className="p-5 bg-slate-50/50 border-t border-slate-100">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-3">
                        {/* Placeholder jika Anda punya data tanggal/waktu dari API */}
                        <div className="flex items-center justify-between sm:justify-start gap-2 text-sm">
                          <span className="flex items-center gap-2 text-slate-500 font-medium sm:w-32">

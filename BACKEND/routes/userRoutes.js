...
router.post('/bookings', async (req, res) => {
  try {
    ...
    await dbRun(
      `INSERT INTO bookings (order_id, user_id, item_type, item_name, location, start_date, end_date, total_price, status, payment_status) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', 'paid')`,
      [order_id, req.user.id, item_type, item_name, location, start_date, end_date, parsedPrice]
    );
...
router.put('/bookings/:orderId/extend', async (req, res) => {
  try {
    ...
    const booking = await dbGet(
      `SELECT start_date, end_date, total_price, status FROM bookings WHERE order_id = ? AND user_id = ?`,
      [orderId, req.user.id]
    );
